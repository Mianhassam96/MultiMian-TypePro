      ...
      <div className="rounded-3xl overflow-hidden surface">
        ...
        <div className="p-6">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            ...
            <span className="ml-auto text-secondary text-sm self-center">{filtered.length} entries</span>
          </div>

          {filtered.length === 0 ? (
            <div className="text-center py-16">
              <div className="text-6xl mb-4 animate-float">🏅</div>
              <p className="text-primary text-lg font-medium">No scores yet</p>
              <p className="text-muted text-sm mt-1">Complete a test and save your score to appear here!</p>
            </div>
          ) : (
              ...
              {filtered.slice(0, 50).map((score, i) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-primary font-semibold truncate">{score.username || 'Anonymous'}</p>
                    <p className="text-muted text-xs">{fmt(score.timestamp)}</p>
                  </div>

                  <div className="text-center">
                    <div className="text-accent font-black text-xl tabular-nums">{score.wpm}</div>
                    <div className="text-muted text-xs">WPM</div>
                  </div>

                  <div className="text-center hidden sm:block">
                    <div className="text-emerald-500 font-bold text-lg tabular-nums">{score.accuracy}%</div>
                    <div className="text-muted text-xs">Acc</div>
                  </div>
                  ...
                  <span className="hidden md:inline-flex px-2.5 py-1 rounded-lg text-xs font-semibold text-blue-500 bg-blue-500/10">
                    {score.timeMode}
                  </span>
                </div>
              ))}
